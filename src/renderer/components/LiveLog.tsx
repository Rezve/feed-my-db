...
const LiveLog: React.FC = () => {
  ...
  return (
    <div
      className={`log-section ${isLogsOpen ? 'open' : 'closed'} bg-white border border-gray-300 rounded-md shadow-sm`}
    >
      {/* Section Header */}
      ...
      {isLogsOpen && (
        <div className="section-content flex flex-col md:flex-row">
          ...
          <div className="w-full md:w-1/2 p-4 border-r border-gray-300">
            <div className="log-container bg-white rounded-md h-64 overflow-y-auto" ref={logContainerRef}>
              {logs.map((log) => (
                <div
                  key={log.id}
                  className="log-entry flex space-x-2 text-sm border-b border-gray-200 py-1 last:border-b-0 px-2"
                >
                  <span className="timestamp text-gray-500 font-mono min-w-[120px]">{log.timestamp}</span>
